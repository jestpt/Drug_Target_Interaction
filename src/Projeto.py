...
# Kruskal-Wallis
def Kruskal_Wallis(data,labels):
    data_H = []
    data_Pvalues = []
...
def plot_rf(features_importance):
	figure=plt.figure()
...
def features_selection(data_rfe,data_rf,data_kw,number_features):
    scaler = MinMaxScaler()
...
def RFE_LinearSVC(data,labels,step_value):
...
def rf_classifier(data,labels,n):
...
def LinearSVM(data,labels,cross):
    linearsvc=LinearSVC()
    if cross==False:
...
def rf_classifier(data,labels,n,cross):
...
def SVC_nonlinear(data,labels,kernel,cross,c_value,poly_degree):
...
def KNN(data,labels,num_neigh,cross):
